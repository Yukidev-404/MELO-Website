...
function dashboardHtml(d,releases){
  const m=d.metrics||{},a=d.adoption||{},p=a.platforms||[],v=a.versions||[],rows=d.recentActivity||[],releaseRows=releases?.rows||releases?.releases||[],active=releaseRows.find(r=>String(r.release_status||'').toLowerCase()==='active')||releaseRows[0],staged=releaseRows.find(r=>String(r.release_status||'').toLowerCase()==='staged');
  ...
  <div class="subgrid"><section class="mini-panel"><h3>Release Pipeline</h3><div class="kv"><span>Active</span><b>${esc(activeLabel)}</b></div><div class="kv"><span>Status</span><b>${esc(releaseStatus)}</b></div><div class="kv"><span>Staged</span><b>${esc(stagedLabel)}</b></div><a class="dashboard-link" href="/admin/?view=releases">OPEN RELEASES ↗</a></section>
