...
const views = {
  ...
  console:{title:'Console',heading:'Control Room Console',copy:'Read-only operations terminal · live telemetry stream'}
};
...
const specializedViews=new Set(['users','installations','bug-reports','crashes','security','services','chat','releases','flags','analytics','health','settings','console']);
function scheduleRefresh(){clearTimeout(refreshTimer);refreshTimer=setTimeout(()=>{if(currentView==='console')return;loadSpecialized(currentView)},30)}
async function loadSpecialized(view){if(view==='console')return;}
function render(view,search=''){currentView=views[view]?view:'dashboard';document.querySelectorAll('.nav-item').forEach(a=>a.classList.toggle('active',a.dataset.view===currentView));title.textContent=views[currentView].title;history.replaceState(null,'',`/admin/?view=${currentView}${search?`&q=${encodeURIComponent(search)}`:''}`);if(currentView==='console'){content.innerHTML='';const s=document.createElement('script');s.src=`console.js?v=${Date.now()}`;s.onload=()=>{};document.body.appendChild(s);return;}if(specializedViews.has(currentView)){scheduleRefresh();return;}content.innerHTML=`<section class="page"><div class="page-intro"><h2>${esc(views[currentView].heading)}</h2><p>Loading live control-room data…</p></div></section>`;api(`/api/admin/${currentView}`).then(d=>{content.innerHTML=dashboardHtml(d)}).catch(e=>{content.innerHTML=`<section class="page"><div class="page-intro"><h2>Unable to load ${esc(views[currentView].heading)}</h2><p>${esc(e.message)}</p></div></section>`})}
function boot(){const params=new URLSearchParams(location.search);render(params.get('view')||'dashboard',params.get('q')||'')}
document.querySelectorAll('.nav-item').forEach(a=>a.addEventListener('click',e=>{const v=a.dataset.view;if(v==='console'){e.preventDefault();render('console');return;}e.preventDefault();render(v)}));
