(() => {
  ...
  function resolution(c){
    const resolved=String(c.status||'unresolved').toLowerCase()==='resolved';
    return `<span class="crash-resolution ${resolved?'resolved':'unresolved'}"><i></i>${resolved?'RESOLVED':'UNRESOLVED'}</span>`;
  }
  function close(){document.getElementById('crash-detail-overlay')?.remove()}
  function exportCsv(rows){
    const h=['Crash ID','Installation ID','Version','Build','Platform','OS','Error Type','Severity','Status','Message','Timestamp'];
    const csv=[h,...rows.map(r=>[r.crash_id,r.installation_id,r.app_version,r.build,r.platform,r.os_version,r.error_type,r.severity,r.status||'unresolved',r.message,date(r.timestamp)])].map(row=>row.map(v=>`"${String(v??'').replace(/"/g,'""')}"`).join(','));
  ...
  async function resolveCrash(c, button){
    if(String(c.status||'unresolved').toLowerCase()==='resolved') return;
    button.disabled=true; button.textContent='RESOLVING…';
    try {
      await api('/api/admin/crash/resolve',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({id:c.crash_id})});
      c.status='resolved'; c.resolved_at=Math.floor(Date.now()/1000);
      close();
      load(new URLSearchParams(location.search).get('q')||'');
    } catch(e) {
      button.disabled=false; button.textContent='MARK RESOLVED';
      alert(e.message || 'Unable to resolve crash.');
    }
  }
  function detail(c){
    const overlay=document.createElement('div'); overlay.id='crash-detail-overlay'; overlay.className='crash-overlay';
    const resolved=String(c.status||'unresolved').toLowerCase()==='resolved';
    overlay.innerHTML=`<div class="crash-modal" role="dialog" aria-modal="true" aria-labelledby="crash-detail-title">
      <header class="crash-modal-head"><div><div class="crash-eyebrow">CRASH INVESTIGATION</div><div class="crash-title"><span class="crash-title-dot"></span><h2 id="crash-detail-title">${esc(c.error_type||'Unhandled crash')}</h2></div><p>${esc(c.crash_id)}</p></div><div class="crash-head-actions">${resolution(c)}<button class="crash-close" type="button">CLOSE</button></div></header>
      <div class="crash-modal-body">
        <div class="crash-hero-stats"><div><span>SEVERITY</span>${severity(c)}</div><div><span>STATUS</span>${resolution(c)}</div><div><span>VERSION</span><strong>${esc(c.app_version||'—')}</strong><small>build ${esc(c.build||'—')}</small></div><div><span>OCCURRED</span><strong>${esc(ago(c.timestamp))}</strong><small>${esc(date(c.timestamp))}</small></div></div>
        <div class="crash-context"><div><span>PLATFORM</span><strong>${esc(c.platform||'—')}</strong><small>${esc(c.os_version||'—')}</small></div><div><span>INSTALLATION</span><code>${esc(c.installation_id||'—')}</code></div><div><span>EVENT ID</span><code>${esc(c.event_id||'—')}</code></div></div>
        ...
        <div class="crash-resolution-bar"><div><strong>${resolved?'This crash has been resolved.':'This crash is still unresolved.'}</strong><span>${resolved?'It no longer contributes to the shared Control Room notification.':'Resolving it removes the unread indicator for every administrator.'}</span></div>${resolved?'':`<button class="crash-resolve" type="button">MARK RESOLVED</button>`}</div>
    ...
    overlay.querySelector('.crash-resolve')?.addEventListener('click',e=>resolveCrash(c,e.currentTarget));
  ...
  function render(rows, query=''){
    ...
    const unresolved=rows.filter(r=>String(r.status||'unresolved').toLowerCase()!=='resolved').length;
    const installations=new Set(rows.map(r=>r.installation_id).filter(Boolean)).size;
    const body=rows.length?rows.map((r,i)=>`<tr class="data-row crash-row ${String(r.status||'unresolved').toLowerCase()==='resolved'?'is-resolved':''}" data-index="${i}">
      ...
      <td>${severity(r)}<small class="cell-sub">${resolution(r)}</small></td>
    ...
    content.innerHTML=`<section class="page crash-page"><div class="page-intro crash-intro"><div><div class="crash-kicker">MONITORING / DIAGNOSTICS</div><h2>Crash Investigation</h2><p>Sanitized crash telemetry from MELO clients. Select a report to inspect the failure context.</p></div><span class="crash-live"><i></i>LIVE PIPELINE</span></div>
      ...
      <section class="panel crash-panel"><div class="panel-head"><div><h2>Crash Reports</h2></div><span>${rows.length} REPORT${rows.length===1?'':'S'} · LIVE D1</span></div><table class="data-table crash-table"><thead><tr><th>FAILURE</th><th>VERSION</th><th>PLATFORM / OS</th><th>SEVERITY / STATUS</th><th>TIME</th><th>INSTALLATION</th></tr></thead><tbody>${body}</tbody></table></section>
      <div class="health-strip crash-strip"><span><strong>SHARED CONTROL ROOM STATE.</strong> A crash stays unread for every administrator until it is resolved.</span><span class="right">CLICK A REPORT TO INVESTIGATE</span></div></section>`;
  ...
  async function load(query=''){
    try{const d=await api('/api/admin/crashes'+(query?`?q=${encodeURIComponent(query)}`:''));render(d.rows||[],query)}catch(e){content.innerHTML=`<section class="page"><div class="page-intro"><h2>Unable to load crashes</h2><p>${esc(e.message)}</p></div></section>`}
  }
