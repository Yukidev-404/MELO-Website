(() => {
  ...
  const imported=[]; let importedIndex=-1; let objectUrls=[]; let musicInput=null;
  ...
  function startLoading(){
    ...
    requestAnimationFrame(tick); setTimeout(()=>loading?.classList.add('ring-on'),300); setTimeout(()=>loading?.classList.add('accent'),1000);
  ...
  function setVisualTrack(name,who){if(title)title.textContent=name;if(artist)artist.textContent=who}
  ...
  function updateProgress(){
    if(!bar)return; const pct=audio.duration?audio.currentTime/audio.duration*100:0; const fill=bar.querySelector('i'); if(fill)fill.style.width=pct+'%';
    const times=bar.parentElement?.querySelector('.time'); if(times){const spans=times.querySelectorAll('span');if(spans.length>1){spans[0].textContent=formatTime(audio.currentTime);spans[1].textContent=formatTime(audio.duration)}}
  }
  function playImported(i){
    if(!imported.length)return; importedIndex=(i+imported.length)%imported.length; const item=imported[importedIndex];
  ...
  function renderImported(){
    const list=document.querySelector('[data-room="library"] .room-list');if(!list)return;list.querySelectorAll('.melo-imported-row').forEach(x=>x.remove());
    imported.forEach((item,i)=>{const row=document.createElement('div');row.className='room-row melo-imported-row';row.innerHTML=`<b>${String(i+5).padStart(2,'0')}</b><div><strong></strong><small>your browser file · ${item.type||'audio'}</small></div><button type="button">PLAY</button>`;row.querySelector('strong').textContent=item.name;row.querySelector('button').addEventListener('click',()=>playImported(i));list.appendChild(row)})
  }
  function importFiles(files){
    [...files].filter(f=>f.type.startsWith('audio/')||/\.(mp3|wav|ogg|m4a|aac|flac)$/i.test(f.name)).forEach(file=>{const url=URL.createObjectURL(file);objectUrls.push(url);const base=file.name.replace(/\.[^.]+$/,'').replace(/[_-]+/g,' ');imported.push({name:base,artist:'Local file',url,type:file.type||'audio'})});
    renderImported(); if(imported.length&&importedIndex<0)playImported(0); if(musicInput)musicInput.value='';
  }
  function setupWebPlayer(){
    const library=document.querySelector('[data-room="library"]');if(!library)return;
    const controls=document.createElement('div');controls.className='melo-library-tools';controls.innerHTML='<button type="button" class="melo-import-btn">＋ ADD MUSIC</button><span class="melo-library-status">browser library · local files stay on this device</span>';
    const list=library.querySelector('.room-list');list?.before(controls);
    Object.assign(controls.style,{display:'flex',alignItems:'center',gap:'10px',margin:'0 0 14px',flexWrap:'wrap'});
    const add=controls.querySelector('.melo-import-btn');Object.assign(add.style,{border:'1px solid var(--ink)',background:'var(--ink)',color:'#fff',padding:'8px 11px',fontSize:'8px',cursor:'pointer'});
    const status=controls.querySelector('.melo-library-status');Object.assign(status.style,{fontSize:'7px',color:'#888'});
    musicInput=document.createElement('input');musicInput.type='file';musicInput.accept='audio/*';musicInput.multiple=true;musicInput.hidden=true;musicInput.setAttribute('aria-label','Choose music files');document.body.appendChild(musicInput);
    add.addEventListener('click',chooseMusic);musicInput.addEventListener('change',e=>importFiles(e.target.files));window.addEventListener('beforeunload',()=>objectUrls.forEach(URL.revokeObjectURL));
  }
  ...
  entry?.addEventListener('blur',()=>{if(!entering)inner?.classList.remove('rim-hover')});entry?.addEventListener('pointerdown',()=>inner?.classList.add('rim-active'));entry?.addEventListener('click',enter);
  document.addEventListener('keydown',e=>{if(e.key==='Enter'&&!boot.classList.contains('hide'))enter();if(e.key==='Escape')rooms.forEach(r=>r.classList.remove('open'));if(e.key==='ArrowRight'&&imported.length)playImported(importedIndex+1);if(e.key==='ArrowLeft'&&imported.length)playImported(importedIndex-1);if(e.key===' '&&boot.classList.contains('hide')){e.preventDefault();togglePlayback()}});
  ...
  document.getElementById('nextTrack')?.addEventListener('click',()=>imported.length?playImported(importedIndex+1):chooseMusic());document.getElementById('prevTrack')?.addEventListener('click',()=>imported.length?playImported(importedIndex-1):chooseMusic());
  ...
  audio.addEventListener('timeupdate',updateProgress);audio.addEventListener('loadedmetadata',updateProgress);audio.addEventListener('play',()=>{playing=true;updatePlayButton()});audio.addEventListener('pause',()=>{playing=false;updatePlayButton()});audio.addEventListener('ended',()=>{if(imported.length)playImported(importedIndex+1);else{playing=false;updatePlayButton()}});
