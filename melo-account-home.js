...
async function load(){
  try{
    ...
    if(!r.ok||!data.authenticated||!data.user){
      box.classList.add('logged-out');
      name.textContent='NOT CONNECTED';email.textContent='SIGN IN TO MELO';provider.textContent='No account connected';action.textContent='SIGN IN →';action.href='login.html';return;
    }
    const u=data.user;
    box.classList.add('connected');
    name.textContent=u.display_name||u.username||u.name||u.email?.split('@')[0]||'MELO PLAYER';
    email.textContent=u.email||u.username||'MELO ACCOUNT';
    provider.textContent=`CONNECTED VIA ${providerName(u).toUpperCase()}`;
    action.textContent='ACCOUNT →';action.href='melo-card.html';
    ...
    box.classList.add('logged-out');name.textContent='ACCOUNT OFFLINE';email.textContent='TRY AGAIN';provider.textContent='Could not check connection';action.textContent='SIGN IN →';action.href='login.html';
  }
}
load();
