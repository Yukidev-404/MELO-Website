...
async function telemetryInstall(body, env, request) {
  ...
  try {
    ...
    if (String(error?.message || "").toLowerCase().includes("unique")) return json({ error: "Installation is already registered." }, 409);
    throw error;
  }
...
async function installRateLimited(env, ip) {
  const key = `telemetry-install:${ip}`;
  const now = Math.floor(Date.now() / 1000);
  const window = 10 * 60;
  const max = 10;
  const row = await env.DB.prepare("SELECT window_start, count FROM admin_attempts WHERE key = ?").bind(key).first();
...
function sanitizeText(value, max) {
  return String(value || "").replace(/[\u0000-\u0008\u000B\u000C\u000E-\u001F\u007F]/g, "").slice(0, max).replace(/(?:Bearer\s+)[A-Za-z0-9._~+/=-]+/gi, "Bearer [REDACTED]").replace(/(?:access[_ -]?token|refresh[_ -]?token|api[_ -]?key|password|secret)\s*[:=]\s*[^\s,;]+/gi, "$1=[REDACTED]").replace(/([A-Za-z]:\\Users\\)[^\\]+/gi, "$1[REDACTED]").replace(/(\/Users\/|\/home\/)[^\/]+/gi, "$1[REDACTED]");
}
...
async function completeSetupOrLogin(request, env, url, body, isSetup) {
  ...
  const now = Math.floor(Date.now() / 1000);
  if (isSetup) await env.DB.prepare(`UPDATE admin_config SET totp_secret_enc=pending_secret_enc, pending_secret_enc=NULL, pending_expires_at=NULL, setup_complete=1, updated_at=? WHERE id=1`).bind(now).run();
  else if (!row.setup_complete) await env.DB.prepare("UPDATE admin_config SET setup_complete=1, updated_at=? WHERE id=1").bind(now).run();
...
async function createSessionResponse(env, origin) {
  const raw = randomToken(32); const hash = await sha256Hex(raw); const now = Math.floor(Date.now() / 1000); const expires = now + SESSION_TTL;
  await env.DB.prepare("INSERT INTO admin_sessions (token_hash, created_at, expires_at) VALUES (?, ?, ?)").bind(hash, now, expires).run();
  const headers = new Headers({ "Content-Type": "application/json", "Cache-Control": "no-store" }); headers.append("Set-Cookie", `${SESSION_COOKIE}=${raw}; Max-Age=${SESSION_TTL}; Path=/; HttpOnly; Secure; SameSite=Strict`);
  return new Response(JSON.stringify({ ok: true, redirect: "/admin/" }), { status: 200, headers });
}
async function getSession(request, env) {
  const cookie = request.headers.get("Cookie") || ""; const match = cookie.match(new RegExp(`(?:^|;\\s*)${SESSION_COOKIE}=([^;]+)`)); if (!match) return null;
  const hash = await sha256Hex(match[1]); const row = await env.DB.prepare("SELECT token_hash, expires_at FROM admin_sessions WHERE token_hash=?").bind(hash).first(); if (!row) return null;
  if (row.expires_at <= Math.floor(Date.now() / 1000)) { await env.DB.prepare("DELETE FROM admin_sessions WHERE token_hash=?").bind(hash).run(); return null; } return row;
}
...
async function logout(request, env, url) {
  if (!sameOrigin(request, url)) return json({ error: "Invalid origin." }, 403); const cookie = request.headers.get("Cookie") || ""; const match = cookie.match(new RegExp(`(?:^|;\\s*)${SESSION_COOKIE}=([^;]+)`));
  if (match) { const hash = await sha256Hex(match[1]); await env.DB.prepare("DELETE FROM admin_sessions WHERE token_hash=?").bind(hash).run(); }
  const headers = new Headers({ "Content-Type": "application/json", "Cache-Control": "no-store" }); headers.set("Set-Cookie", `${SESSION_COOKIE}=; Max-Age=0; Path=/; HttpOnly; Secure; SameSite=Strict`); return new Response(JSON.stringify({ ok: true }), { status: 200, headers });
}
async function rateLimited(env, key) {
  const now = Math.floor(Date.now() / 1000); const row = await env.DB.prepare("SELECT window_start,count FROM admin_attempts WHERE key=?").bind(key).first();
  if (!row || now - row.window_start >= RATE_WINDOW) { await env.DB.prepare("INSERT INTO admin_attempts (key,window_start,count) VALUES (?, ?, 1) ON CONFLICT(key) DO UPDATE SET window_start=excluded.window_start,count=1").bind(key, now).run(); return false; }
  if (row.count >= MAX_ATTEMPTS) return true; await env.DB.prepare("UPDATE admin_attempts SET count=count+1 WHERE key=?").bind(key).run(); return false;
}
