...
async function handleUpdates(request, env, url) {
  if (url.pathname !== "/api/updates/check" || request.method !== "GET") return json({ error: "Not found." }, 404);
  const platform = String(url.searchParams.get("platform") || "windows").trim().toLowerCase();
  const currentVersion = String(url.searchParams.get("version") || "0.0.0").trim();
  const currentBuild = String(url.searchParams.get("build") || "").trim();
  if (!ALLOWED_PLATFORMS.has(platform)) return json({ update: false });
  if (!/^\d{1,32}(?:\.\d{1,32}){0,3}$/.test(currentVersion)) return json({ update: false });

  const latest = await env.DB.prepare("SELECT version, build, platform, release_status, release_notes, released_at FROM releases WHERE platform=? AND release_status IN ('active','staged') ORDER BY released_at DESC LIMIT 20").bind(platform).all();
  const candidates = (latest?.results || []).filter((release) => compareVersions(String(release.version || "0.0.0"), currentVersion) > 0);
  if (!candidates.length) return json({ update: false, current: { version: currentVersion, build: currentBuild } });
...
function compareVersions(a, b) {
  const left = String(a || "0").split(".").map((v) => Number.parseInt(v, 10) || 0);
  const right = String(b || "0").split(".").map((v) => Number.parseInt(v, 10) || 0);
...
async function handleApi(request, env, url) {
  ...
  if (url.pathname === "/api/admin/bug-attachment" && request.method === "GET") {
    const session = await getSession(request, env);
    if (!session) return json({ authenticated: false }, 401);
...
async function bugReportAttachment(request, env, url) {
...
async function updateBugReport(request,env,url) {
  ...
  if(!/^bug_[A-Za-z0-9_-]{16,64}$/.test(id)) return json({error:'Invalid report ID.'},400);
  if(!['new','reviewing','resolved','dismissed'].includes(status)) return json({error:'Invalid report status.'},400);
  const now=Math.floor(Date.now()/1000);
...
async function dashboardData(env) {
  const now=Math.floor(Date.now()/1000), dayStart=now-(now%86400), weekAgo=now-7*86400;
  ...
  const currentVersion=latestRelease?.version||versions?.results?.[0]?.app_version||"—"; const currentCount=versions?.results?.find(r=>r.app_version===currentVersion)?.count||0; const totalCount=Number(total?.count||0);
...
async function installationsData(env,search) {
  const pattern=`%${search.replace(/[%_]/g,"\\$&" )}%`; let result;
...
async function securityData(env){const now=Math.floor(Date.now()/1000); const [attempts,config,sessions]=await Promise.all([env.DB.prepare("SELECT key, window_start, count FROM admin_attempts WHERE window_start >= ? ORDER BY window_start DESC LIMIT 100").bind(now-RATE_WINDOW).all(),env.DB.prepare("SELECT email, setup_complete, created_at, updated_at FROM admin_config WHERE id=1").first(),env.DB.prepare("SELECT COUNT(*) AS count FROM admin_sessions WHERE expires_at > ?").bind(now).first()]); return json({ok:true,attempts:attempts?.results||[],admin:{email:config?.email||ADMIN_EMAIL,setupComplete:Boolean(config?.setup_complete),activeSessions:Number(sessions?.count||0)}});}
async function servicesData(env){const started=Date.now();let db={status:"DOWN",latencyMs:null};try{await env.DB.prepare("SELECT 1 AS ok").first();db={status:"OPERATIONAL",latencyMs:Date.now()-started};}catch{} return json({ok:true,services:[{service:"MELO Worker",status:"OPERATIONAL",latencyMs:0,note:"Current Worker request is responding."},{service:"D1 Database",status:db.status,latencyMs:db.latencyMs,note:"melo-admin database connectivity."},{service:"Telemetry API",status:"OPERATIONAL",latencyMs:0,note:"Telemetry endpoints are deployed."},{service:"Spotify API",status:"NOT MONITORED",latencyMs:null,note:"Spotify service monitoring is not connected yet."}]});}
...
async function analyticsData(env){const now=Math.floor(Date.now()/1000);const [events,versions,platforms,daily]=await Promise.all([env.DB.prepare("SELECT event_type, COUNT(*) AS count FROM telemetry_events GROUP BY event_type ORDER BY count DESC").all(),env.DB.prepare("SELECT app_version, COUNT(*) AS count FROM installations GROUP BY app_version ORDER BY count DESC").all(),env.DB.prepare("SELECT platform, COUNT(*) AS count FROM installations GROUP BY platform ORDER BY count DESC").all(),env.DB.prepare("SELECT (created_at / 86400) AS day, COUNT(*) AS count FROM installations WHERE created_at >= ? GROUP BY day ORDER BY day DESC LIMIT 30").bind(now-30*86400).all()]);return json({ok:true,events:events?.results||[],versions:versions?.results||[],platforms:platforms?.results||[],dailyInstallations:daily?.results||[]});}
async function healthData(env){const now=Math.floor(Date.now()/1000);const [installations,events,crashes,lastEvent,lastCrash]=await Promise.all([env.DB.prepare("SELECT COUNT(*) AS count FROM installations").first(),env.DB.prepare("SELECT COUNT(*) AS count FROM telemetry_events").first(),env.DB.prepare("SELECT COUNT(*) AS count FROM crash_reports").first(),env.DB.prepare("SELECT timestamp, event_type FROM telemetry_events ORDER BY timestamp DESC LIMIT 1").first(),env.DB.prepare("SELECT timestamp, severity FROM crash_reports ORDER BY timestamp DESC LIMIT 1").first()]);return json({ok:true,checkedAt:now,components:[{component:"Worker",status:"OPERATIONAL",detail:"Request handler active."},{component:"D1",status:"OPERATIONAL",detail:`${Number(installations?.count||0)} installations / ${Number(events?.count||0)} events / ${Number(crashes?.count||0)} crashes`},{component:"Telemetry ingestion",status:"OPERATIONAL",detail:lastEvent?`Last ${lastEvent.event_type} event recorded.`:"No telemetry events yet."},{component:"Crash reporting",status:"OPERATIONAL",detail:lastCrash?`Last ${lastCrash.severity} crash recorded.`:"No crash reports yet."}]});}
async function settingsData(env){const row=await env.DB.prepare("SELECT email, setup_complete, created_at, updated_at FROM admin_config WHERE id=1").first();return json({ok:true,settings:[{setting:"Authenticator 2FA",value:row?.setup_complete?"Active":"Not configured",status:row?.setup_complete?"Protected":"Action required"},{setting:"Session TTL",value:`${SESSION_TTL/3600} hours`,status:"Active"},{setting:"Admin account",value:row?.email||ADMIN_EMAIL,status:"Protected"},{setting:"Telemetry schema",value:"v1",status:"Active"}]});}
...
async function telemetryInstall(body,env,request){if(body.event_type!=="install")return json({error:"Install endpoint requires event_type=install."},400);const ip=request.headers.get("CF-Connecting-IP")||"unknown";if(await installRateLimited(env,ip))return json({error:"Too many installation requests. Try again later."},429);const existing=await env.DB.prepare("SELECT installation_id FROM installations WHERE installation_id = ?").bind(body.installation_id).first();if(existing)return json({error:"Installation is already registered."},409);const now=Math.floor(Date.now()/1e3);const credential=randomToken(32);const hash=await sha256Hex(credential);const statements=[env.DB.prepare(`INSERT INTO installations (installation_id, app_version, build, platform, os_version, client_schema, first_seen, last_seen, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).bind(body.installation_id,body.app_version,body.build||null,body.platform,body.os_version||null,body.client_schema,body.timestamp,body.timestamp,now,now),env.DB.prepare(`INSERT INTO telemetry_events (event_id, installation_id, event_type, timestamp, app_version, build, platform, os_version, client_schema, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).bind(body.event_id,body.installation_id,body.event_type,body.timestamp,body.app_version,body.build||null,body.platform,body.os_version||null,body.client_schema,now),env.DB.prepare(`INSERT INTO telemetry_credentials (installation_id, token_hash, created_at, last_used_at) VALUES (?, ?, ?, ?)`).bind(body.installation_id,hash,now,now)];try{await env.DB.batch(statements);}catch(error){if(String(error?.message||"").toLowerCase().includes("unique"))return json({error:"Installation is already registered."},409);throw error;}return json({ok:true,registered:true,telemetry_token:credential,installation_id:body.installation_id,server_time:now});}
...
function validateTelemetry(body){if(!body||typeof body!=="object"||Array.isArray(body))return{ok:false,error:"Invalid JSON payload."};const id=String(body.installation_id||"");const eventId=String(body.event_id||"");const type=String(body.event_type||"");const timestamp=Number(body.timestamp);const version=String(body.app_version||"");const build=body.build==null?"":String(body.build);const platform=String(body.platform||"");const os=body.os_version==null?"":String(body.os_version);const schema=Number(body.client_schema);if(!/^[A-Za-z0-9_-]{16,128}$/.test(id))return{ok:false,error:"Invalid installation_id."};if(!/^[A-Za-z0-9_-]{16,128}$/.test(eventId))return{ok:false,error:"Invalid event_id."};if(!ALLOWED_EVENT_TYPES.has(type))return{ok:false,error:"Invalid event_type."};if(!Number.isInteger(timestamp)||timestamp<1||timestamp>Math.floor(Date.now()/1000)+300)return{ok:false,error:"Invalid timestamp."};if(!/^\d{1,32}(?:\.\d{1,32}){0,3}$/.test(version)||version.length>64)return{ok:false,error:"Invalid app_version."};if(build.length>64||platform.length>32||os.length>128)return{ok:false,error:"Telemetry field too long."};if(!ALLOWED_PLATFORMS.has(platform))return{ok:false,error:"Unsupported platform."};if(!Number.isInteger(schema)||schema!==1)return{ok:false,error:"Unsupported client_schema."};if(type==="crash"){const crash=body.crash;if(!crash||typeof crash!=="object"||Array.isArray(crash))return{ok:false,error:"Invalid crash payload."};if(String(crash.error_type||"").length>128||String(crash.message||"").length>4096||String(crash.stack_trace||"").length>16384)return{ok:false,error:"Crash payload too large."};if(!["error","fatal","warning"].includes(String(crash.severity||"error")))return{ok:false,error:"Invalid crash severity."};}return{ok:true};}
async function recordTelemetry(body,env,auth){const now=Math.floor(Date.now()/1000);const crashId=body.event_type==="crash"?randomToken(18):null;const statements=[env.DB.prepare(`INSERT INTO telemetry_events (event_id,installation_id,event_type,timestamp,app_version,build,platform,os_version,client_schema,created_at) VALUES (?,?,?,?,?,?,?,?,?,?)`).bind(body.event_id,body.installation_id,body.event_type,body.timestamp,body.app_version,body.build||null,body.platform,body.os_version||null,body.client_schema,now),env.DB.prepare(`UPDATE installations SET app_version=?,build=?,platform=?,os_version=?,client_schema=?,last_seen=?,updated_at=? WHERE installation_id=?`).bind(body.app_version,body.build||null,body.platform,body.os_version||null,body.client_schema,body.timestamp,now,body.installation_id),env.DB.prepare("UPDATE telemetry_credentials SET last_used_at=? WHERE installation_id=?").bind(now,auth.installation_id)];if(body.event_type==="crash"){const crash=body.crash;statements.push(env.DB.prepare(`INSERT INTO crash_reports (crash_id,event_id,installation_id,error_type,message,stack_trace,severity,app_version,build,platform,os_version,timestamp,created_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)`).bind(crashId,body.event_id,body.installation_id,sanitizeText(crash.error_type,128),sanitizeText(crash.message,4096),sanitizeStack(crash.stack_trace,16384),crash.severity||"error",body.app_version,body.build||null,body.platform,body.os_version||null,body.timestamp,now));}try{await env.DB.batch(statements);}catch(error){if(String(error?.message||"").toLowerCase().includes("unique"))return json({ok:true,duplicate:true});throw error;}return json({ok:true,recorded:true});}
