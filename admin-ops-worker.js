...
async function releasePublish(request, env, session) {
  ...
  if (before) {
    await env.DB.prepare('UPDATE releases SET release_status=?,release_notes=?,download_url=?,released_at=?,updated_at=? WHERE version=? AND build=? AND platform=?').bind(status, notes || null, downloadUrl || null, now, now, version, build, platform).run();
  } else {
    await env.DB.prepare('INSERT INTO releases (version,build,platform,release_status,release_notes,download_url,released_at,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?)').bind(version, build, platform, status, notes || null, downloadUrl || null, now, now, now).run();
  }
