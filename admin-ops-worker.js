...
export async function audit(env, session, request, action, resourceType, resourceId, details = {}) {
  try {
    ...
    console.error('MELO audit write failed', error);
  }
}
...
async function releasePublish(request, env, session) {
  if (session.role !== OWNER || session.username !== OWNER_NAME) return json({ error: 'Owner access required for release publishing.' }, 403);
  let body;
  try { body = await request.json(); } catch { return json({ error: 'Invalid JSON payload.' }, 400); }
  const version = String(body?.version || '').trim();
  const build = String(body?.build || '').trim();
  const platform = String(body?.platform || 'windows').trim().toLowerCase();
  const notes = String(body?.release_notes || '').trim().slice(0, 12000);
  const downloadUrl = String(body?.download_url || '').trim();
  const status = String(body?.status || 'staged').trim().toLowerCase();
  if (!/^\d{1,32}(?:\.\d{1,32}){0,3}$/.test(version)) return json({ error: 'Invalid release version.' }, 400);
  if (build.length > 128) return json({ error: 'Build is too long.' }, 400);
  if (platform !== 'windows') return json({ error: 'Only Windows releases are supported currently.' }, 400);
  if (!['active', 'staged', 'archived'].includes(status)) return json({ error: 'Invalid release status.' }, 400);
  if (downloadUrl && !/^https:\/\//i.test(downloadUrl)) return json({ error: 'Download URL must use HTTPS.' }, 400);
  await ensureReleaseSchema(env);
  const now = Math.floor(Date.now() / 1000);
  const before = await env.DB.prepare('SELECT release_status,release_notes,download_url FROM releases WHERE version=? AND build=? AND platform=?').bind(version, build, platform).first();
  ...
  await env.DB.prepare(`INSERT INTO releases (version,build,platform,release_status,release_notes,download_url,released_at,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?) ON CONFLICT(version,build,platform) DO UPDATE SET release_status=excluded.release_status,release_notes=excluded.release_notes,download_url=excluded.download_url,released_at=excluded.released_at,updated_at=excluded.updated_at`).bind(version, build, platform, status, notes || null, downloadUrl || null, now, before ? undefined : now, now).run();
  await audit(env, session, request, `release.publish.${status}`, 'release', `${platform}:${version}:${build}`, { beforeStatus: String(before?.release_status || 'none'), afterStatus: status, hasDownloadUrl: Boolean(downloadUrl) });
  const row = await env.DB.prepare('SELECT version,build,platform,release_status,release_notes,download_url,released_at,created_at,updated_at FROM releases WHERE version=? AND build=? AND platform=?').bind(version, build, platform).first();
  return json({ ok: true, release: row });
}
