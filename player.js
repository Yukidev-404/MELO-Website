...
async function tabLoad(){if(s.source==='LOCAL'){s.tracks=s.localTracks.slice();if(s.tab==='FAVORITES')s.tracks=s.favorites.slice();if(s.tab==='RECENT')s.tracks=s.recent.slice();if(s.tab==='QUEUE')s.tracks=s.queue.slice();render();return}try{if(s.tab==='LIBRARY'||s.tab==='FAVORITES'){const d=await api('/me/tracks?limit=50');s.tracks=(d.items||[]).map(x=>track(x.track)).filter(Boolean);s.favorites=s.tracks.slice()}else if(s.tab==='RECENT'){const d=await api('/me/player/recently-played?limit=50');s.tracks=(d.items||[]).map(x=>track(x.track)).filter(Boolean)}else if(s.tab==='QUEUE'){const d=await api('/me/player/queue');s.tracks=[track(d.current_track),...(d.queue||[]).map(track)].filter(Boolean)}else if(s.tab==='PLAYLISTS'){const d=await api('/me/playlists?limit=50');s.playlists=(d.items||[]).map(p=>({id:p.id,name:p.name,total:p.items?.total||0,image:p.images?.[0]?.url||'',owner:p.owner?.display_name||p.owner?.id||'Spotify'}));s.tracks=[]}render()}catch(e){$('sourceStatus').textContent='OFFLINE';content.innerHTML=`<div class="empty">${esc(e.message)}</div>`}}function render(){let list=s.tracks||[],q=$('search').value.trim().toLowerCase();if(q)list=list.filter(t=>`${t.name} ${t.artist} ${t.album}`.toLowerCase().includes(q));if(s.tab==='PLAYLISTS'&&s.source==='SPOTIFY'){content.innerHTML=s.playlists.length?`<div class="playlist-grid">${s.playlists.map(p=>`<article class="playlist-card" data-pl="${esc(p.id)}"><div class="playlist-cover">${p.image?`<img src="${esc(p.image)}" alt="">`:'<span>♪</span>'}</div><div class="playlist-card-meta"><div class="playlist-card-title" title="${esc(p.name)}">${esc(p.name)}</div><div class="playlist-card-owner">by ${esc(p.owner||'Spotify')}</div><div class="playlist-card-count">${esc(p.total)} tracks</div></div><button class="playlist-open" type="button">OPEN</button></article>`).join('')}</div>`:'<div class="empty">NO PLAYLISTS</div>';content.querySelectorAll('.playlist-card').forEach(r=>r.onclick=e=>{if(e.target.closest('.playlist-open')){playlist(r.dataset.pl);return}playlist(r.dataset.pl)});return}if(!list.length){content.innerHTML='<div class="empty">NO MUSIC AVAILABLE</div>';return}content.innerHTML=list.map((t,i)=>`<div class="track-row" data-i="${i}"><span class="num">${String(i+1).padStart(2,'0')}</span><div><b>${esc(t.name)}</b><small>${esc(t.artist)} · ${esc(t.album)}</small></div><button data-menu="${esc(t.id)}">⋯</button></div>`).join('');content.querySelectorAll('.track-row').forEach(r=>r.onclick=e=>{if(e.target.closest('button'))return;play(Number(r.dataset.i))});content.querySelectorAll('[data-menu]').forEach(b=>b.onclick=e=>{e.stopPropagation();trackMenu(e.clientX,e.clientY,b.dataset.menu)});content.querySelectorAll('[data-pl]').forEach(r=>r.onclick=()=>playlist(r.dataset.pl))}async function playlist(id){
  try{
